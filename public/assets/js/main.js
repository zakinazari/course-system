...
if (language && langDropdown.length) {
  // پیدا کردن المنت مربوط به زبان انتخاب‌شده
  let selectedDropdownItem = document.querySelector('a[data-language="' + language + '"]');

  // اطمینان از وجود المنت و داشتن حداقل یک child
  if (selectedDropdownItem && selectedDropdownItem.children.length > 0) {
    let flagElement = selectedDropdownItem.children[0]; // اولین المنت واقعی (مثلاً <i>)
    let selectedFlag = flagElement.className;

    let startsWith = 'fs-';
    let classes = selectedFlag.split(' ').filter(function (v) {
      return v.lastIndexOf(startsWith, 0) !== 0;
    });

    selectedFlag = classes.join(' ').trim() + ' fs-3';

    // اضافه کردن کلاس 'selected' به گزینه فعلی
    selectedDropdownItem.classList.add('selected');

    // پیدا کردن dropdown-toggle و اعمال پرچم
    let dropdownToggle = document.querySelector('.dropdown-language .dropdown-toggle');
    if (dropdownToggle && dropdownToggle.children.length > 0) {
      dropdownToggle.children[0].className = selectedFlag;
    }
  } else {
    console.warn('Dropdown item not found or has no child element for language:', language);
  }
}
