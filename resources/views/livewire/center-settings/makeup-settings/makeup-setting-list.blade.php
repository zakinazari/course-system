                ...
                <table class="table">
                    <thead class="table-dark">
                        <tr>
                            <th>{{ __('label.NO') }}</th>
                            <th>{{ __('label.name') }}</th>
                            <th>{{ __('label.exam_valid_days') }}</th>
                            <th>{{ __('label.fee_valid_days') }}</th>
                    ...
                    <tbody class="table-border-bottom-0">
                        @foreach($makeup_settings as $i => $setting)
                        <tr>
                            <td>{{ ($makeup_settings->currentPage() - 1) * $makeup_settings->perPage() + $i + 1 }}</td>
                            <td>{{ $setting->name }}</td>

                            <td>{{ $setting->exam_valid_days }}</td>
                            <td>{{ $setting->fee_valid_days }}</td>
            ...
            <div class="modal-content">
                ...
                <form @if($editMode) wire:submit.prevent="update" @else wire:submit.prevent="store" @endif>
                    <div class="modal-body">
                        ...
                        <div class="row">
                            <div class="col mb-3">
                                <label for="nameBasic" class="form-label">{{ __('label.exam_valid_days') }} <span style="color:red;">*</span></label>
                                <input type="text" id="nameBasic" class="form-control @error('exam_valid_days') is-invalid @enderror" wire:model.lazy="exam_valid_days">
                                @error('exam_valid_days') <div class="invalid-feedback d-block">{{ $message }}</div> @enderror
                            </div>
                            <div class="col mb-3">
                                <label for="nameBasic" class="form-label">{{ __('label.fee_valid_days') }} <span style="color:red;">*</span></label>
                                <input type="text" id="nameBasic" class="form-control @error('fee_valid_days') is-invalid @enderror" wire:model.lazy="fee_valid_days">
                                @error('fee_valid_days') <div class="invalid-feedback d-block">{{ $message }}</div> @enderror
                            </div>
                            <div class="col mb-3">
                                <label for="nameBasic" class="form-label">{{ __('label.fee_amount') }} <span style="color:red;">*</span></label>
                                <input type="text" id="nameBasic" class="form-control @error('fee_amount') is-invalid @enderror" wire:model.lazy="fee_amount">
