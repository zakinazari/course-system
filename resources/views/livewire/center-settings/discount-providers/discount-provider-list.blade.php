                ...
                <table class="table">
                    <thead class="table-dark">
                        <tr>
                            ...
                            <th>{{ __('label.monthly_discount_total') }}</th>
                            <th>{{ __('label.status') }}</th>
                            <th>{{ __('label.actions') }}</th>
                        </tr>
                    </thead>
                    <tbody class="table-border-bottom-0">
                        @foreach($discount_providers as $i => $provider)
                        <tr>
                            <td>{{ ($discount_providers->currentPage() - 1) * $discount_providers->perPage() + $i + 1 }}</td>
                            <td>{{ $provider->name }}</td>
                            <td>{{ $provider->last_name }}</td>
                            <td>{{ $provider->phone_no }}</td>
                            <td>{{ $provider->monthly_discount_total }}</td>
                            <td>
                                @if($provider->status) 
                                ...
                                <div class="dropdown position-static">
                                    ...
                                    <div class="dropdown-menu">
                                        @if(edit(Auth::user()->role_ids,$active_menu_id))
                                            <a class="dropdown-item" href="javascript:void(0);" wire:click="edit({{ $provider->id }})"
                                            ><i class="bx bx-edit-alt me-1 text-success"></i>{{ __('label.edit') }}</a>
                                        @endif
                                        @if(delete(Auth::user()->role_ids,$active_menu_id))
                                            <a class="dropdown-item " href="javascript:void(0);"  onclick="confirmDelete({{ $provider->id }},'{{$table_name}}')"
                ...
                <form @if($editMode) wire:submit.prevent="update" @else wire:submit.prevent="store" @endif>
                    <div class="modal-body">
                        ...
                        <div class="row">
                            ...
                            <div class="col mb-3">
                                <label for="nameBasic" class="form-label">{{ __('label.monthly_discount_total') }} <span style="color:red;">*</span></label>
                                <input type="text" id="nameBasic" class="form-control @error('monthly_discount_total') is-invalid @enderror" wire:model.lazy="monthly_discount_total">
                                @error('monthly_discount_total') <div class="invalid-feedback d-block">{{ $message }}</div> @enderror
                            </div>
                        </div>
