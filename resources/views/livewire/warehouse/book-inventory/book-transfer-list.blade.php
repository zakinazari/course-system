                ...
                <form wire:submit.prevent="applySearch" class="row g-3 align-items-end">
                   @if(!auth()->user()->branch_id)
                    <div class="col-md-3 d-flex flex-column" wire:ignore>
                        ...
                        <select  class="form-select select2" id ="search_central_warehouse_id">
                           <option value="">{{ __('label.all') }}</option>
                           @foreach($central_warehouses as $central)
                                 <option value="{{ $central->id }}"  wire:key="central-warehouse-search-{{ $central->id }}">
                                    {{ $central->name }}
                                 </option>
                           @endforeach
                        </select>
                    ...
                    <div class="col-md-3 d-flex flex-column" wire:ignore>
                        <label class="form-label">{{ __('label.warehouse') }}</label>
                        <select  class="form-select select2" id ="search_branch_warehouse_id">
                           <option value="">{{ __('label.all') }}</option>
                           @foreach($branch_warehouses as $bw)
                                 <option value="{{ $bw->id }}"  wire:key="branch_warehouse-search-{{ $bw->id }}">
                                    {{ $bw->name }}
                                 </option>
                           @endforeach
                        </select>
            ...
            <div class="table-responsive text-nowrap">
                <table class="table">
                    ...
                    <tbody class="table-border-bottom-0">
                        ...
                        <tr>
                            <td>{{ ($transfers->currentPage() - 1) * $transfers->perPage() + $i + 1 }}</td>
                            <td>{{ $transfer->inventory?->warehouse?->name }}</td>
                            <td>{{ $transfer->inventory?->book?->name }}</td>
                            <td>{{ abs($transfer->quantity_change) }}</td>
                            <td>{{ $transfer->balance_after }}</td>
                            ...
                            <td>
                                ...
                                <div class="dropdown position-static">
                                    ...
                                    <div class="dropdown-menu">
                                        ...
                                        @if((Auth::user()->id === $transfer->user_id && $transfer->created_at->format('Y/m/d') == now()->format('Y/m/d')) || Auth::user()->isAdmin() || Auth::user()->isDeveloper())
                                            <a class="dropdown-item" href="javascript:void(0);" wire:click="edit({{ $transfer->id }})"
                                        ...
                                        @if(delete(Auth::user()->role_ids,$active_menu_id))
                                            @if((Auth::user()->id === $transfer->user_id && $transfer->created_at->format('Y/m/d') == now()->format('Y/m/d')) || Auth::user()->isAdmin() || Auth::user()->isDeveloper())
                                            <a class="dropdown-item " href="javascript:void(0);"  onclick="confirmDelete({{ $transfer->id }},'{{$table_name}}')"
                    ...
                    <div class="modal-body">
                        @if(!$editMode)
                            <div class="row">
                                ...
                                wire:change="loadBranchWarehouse($event.target.value)" 
                                id ="branch_id">
                           ...
                           <div class="mb-3">
                                <div >
                                    <label>{{ __('label.branch_warehouse') }}<span style="color:red;">*</span></label>
                                    <select wire:model.lazy="to_warehouse_id" class="form-control select2"
                                                    
                                    id="to_warehouse_id">
                                        <option value="">{{ __('label.select') }}</option>
                                        @foreach($branch_warehouses as $b_warehouse)
                                            <option value="{{ $b_warehouse->id }}">
                                                {{ $b_warehouse->name }}
                                            </option>
                                        @endforeach
                                    </select>
                           ...
                           <div class="mb-3">
                                <div wire:ignore>
                                    <label>{{ __('label.central_warehouse') }}<span style="color:red;">*</span></label>
                                    <select wire:model.lazy="from_warehouse_id" class="form-control select2" id="from_warehouse_id">
                                        <option value="">{{ __('label.select') }}</option>
                                        @foreach($central_warehouses as $c_warehouse)
                                            <option value="{{ $c_warehouse->id }}">
                                                {{ $c_warehouse->name }}
                                            </option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="mb-3" >
                                <label>{{ __('label.physical_book') }}<span style="color:red;">*</span>
                                        @if($available_quantity > 0)

                                            <small class="text-success ms-2">
                                                ({{ __('label.available_quantity') }}: {{ $available_quantity }})
                                            </small>

                                        @elseif($available_quantity !== null && $available_quantity <= 0)

                                            <small class="text-danger ms-2">
                                                ({{ __('label.out_of_stock') }})
                                            </small>
...
document.addEventListener("livewire:initialized", function () {
    ...
    function initSelect2() {
        ...
        $('#search_central_warehouse_id').off('change').on('change', function () {
            $wire.set('search.from_warehouse_id', $(this).val());
        });
        $('#search_branch_warehouse_id').off('change').on('change', function () {
            $wire.set('search.to_warehouse_id', $(this).val());
        });

        $('#branch_id').off('change').on('change', function () {
            $wire.call('loadBranchWarehouse', $(this).val());
        });

        $('#to_warehouse_id').off('change').on('change', function () {
            $wire.set('to_warehouse_id', $(this).val());
        });

        $('#from_warehouse_id').off('change').on('change', function () {
            $wire.set('from_warehouse_id', $(this).val());
        });
