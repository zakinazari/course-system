        ...
        <tbody>
            @foreach($transfers as $i => $transfer)
                <tr>
                    ...
                    @if(in_array('warehouse_id', $selectedFields)) <td>{{ $transfer->inventory?->warehouse?->name }}</td> @endif
                    @if(in_array('book_id', $selectedFields)) <td>{{ $transfer->inventory?->book?->name }}</td> @endif
                    @if(in_array('quantity_change', $selectedFields)) <td>{{ abs($transfer->quantity_change) }}</td> @endif
                    @if(in_array('balance_after', $selectedFields)) <td>{{ $transfer->balance_after }}</td> @endif
                    @if(in_array('created_at', $selectedFields)) <td>{{ $transfer->created_at->format('Y/m/d H:i:A') }}</td> @endif
                </tr>
            @endforeach
        </tbody>
