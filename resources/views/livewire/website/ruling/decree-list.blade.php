                ...
                <table class="table">
                    <thead class="table-dark">
                        <tr>
                            ...
                            <th>{{ __('label.decree.decree_number') }}</th>
                            <th>{{ __('label.decree.decree_date') }}</th>
                    ...
                    <tbody class="table-border-bottom-0">
                        ...
                        <tr>
                            ...
                            <td> {{ $decree->gazette?->gazette_number }} </td>
                            <td>
                                <button type="button" class="btn btn-success btn-icon rounded-pill" wire:click="showFiles({{ $decree->id }})">
                                ...
                                <div class="dropdown position-static">
                                    ...
                                    <div class="dropdown-menu">
                                        @if(edit(Auth::user()->role_ids,$active_menu_id))
                                            <a class="dropdown-item" href="javascript:void(0);" wire:click="edit({{ $decree->id }})"
                                            ><i class="bx bx-edit-alt me-1 text-success"></i>{{ __('label.edit') }}</a>
                                        @endif
                                        @if(delete(Auth::user()->role_ids,$active_menu_id))
                                            <a class="dropdown-item " href="javascript:void(0);"  onclick="confirmDelete({{ $decree->id }},'{{$table_name}}')"
            ...
            <div class="modal-content">
                ...
                <form @if($editMode) wire:submit.prevent="update" @else wire:submit.prevent="store" @endif>
                    <div class="modal-body">
                        ...
                        <div class="row">
                            <div class="col mb-3">
                                <label for="nameBasic" class="form-label">{{ __('label.decree.decree_number') }}</label>
                                <input type="text" id="nameBasic" class="form-control @error('decree_number') is-invalid @enderror" wire:model.lazy="decree_number">
                                @error('decree_number') <div class="invalid-feedback d-block">{{ $message }}</div> @enderror
                            </div>
                         
                            <div class="col mb-3">
                                <label class="form-label">{{ __('label.decree.decree_date') }}</label>

                                <input type="text"
                                    class="form-control @error('decree_date') is-invalid @enderror"
                                    placeholder="{{ hijri::Date('Y/m/d') }}"
                                    wire:model.defer="decree_date">
                                @error('decree_date')
                        ...
                        @foreach($existing_files as $f)
                            <li class="list-group-item d-flex justify-content-between align-items-center">
                                ...
                                <div class="d-flex gap-2">
                                    ...
                                    <button onclick="confirmDelete({{ $f->id }},'decree_files')" type="button" class="btn btn-icon btn-label-danger">
