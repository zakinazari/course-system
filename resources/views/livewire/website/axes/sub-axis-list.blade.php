        ...
        <div class="table-responsive text-nowrap">
            ...
            <div class="mb-3 px-3">
                <form wire:submit.prevent="applySearch" class="row g-3 align-items-end">
                    ...
                    <div class="col-md-4">
                        <label class="form-label" for="multicol-country">{{ __('label.main_axes') }}</label>
                        <select id="multicol-country"  class="form-control"  wire:model="search.main_axis_id">
            ...
            <div class="table-responsive text-nowrap">
                <table class="table">
                    ...
                    <tbody class="table-border-bottom-0">
                        ...
                        <tr>
                            <td>{{ ($sub_axes->currentPage() - 1) * $sub_axes->perPage() + $i + 1 }}</td>
                            <td>
                                 @if(App::getLocale()=='fa') {{ $sub->title_fa }} @else {{ $sub->title_en }} @endif 
                            </td>
                            <td>
                                <div class="dropdown position-static">
                                    ...
                                    <div class="dropdown-menu">
                                        @if(edit(Auth::user()->role_ids,$active_menu_id))
                                            <a class="dropdown-item" href="javascript:void(0);" wire:click="edit({{ $sub->id }})"
                                            ><i class="bx bx-edit-alt me-1 text-success"></i>{{ __('label.edit') }}</a>
                                        @endif
                                        @if(delete(Auth::user()->role_ids,$active_menu_id))
                                            <a class="dropdown-item " href="javascript:void(0);"  onclick="confirmDelete({{ $sub->id }},'{{$table_name}}')"
            ...
            <div class="modal-content">
                ...
                <form @if($editMode) wire:submit.prevent="update" @else wire:submit.prevent="store" @endif>
                    <div class="modal-body">
                        ...
                        <div class="row">
                            <div class="col mb-3">
                                <label class="form-label" for="multicol-country_id">{{ __('label.main_axes') }}</label>
                                <select id="multicol-country_id"  class="form-control @error('main_axis_id') is-invalid @enderror"  wire:model="main_axis_id">
                                    <option value="">{{ __('label.select') }}</option>
                                    @foreach($main_axes as $main)
                                        <option value="{{ $main->id }}" wire:key="main_axis_{{ $main->id }}">
                                            @if(App::getLocale()=='fa')
                                                {{ substr($main->title_fa, 0, 200) }}{{ strlen($main->title_en) > 200 ? '…' : '' }}
                                            @else
                                                {{ substr($main->title_en, 0, 200) }}{{ strlen($main->title_en) > 200 ? '…' : '' }}
