       ...
       <div class="card-header">
        ...
        <div class=" text-nowrap">
            ...
            <div class="text-nowrap table-responsive">
                <table class="table ">
                    <thead class="table-dark">
                        <tr>
                            ...
                            <th>
                                <input class="form-check-input" type="checkbox" wire:model="selectedFields" value="gross_salary">
                            ...
                            <th>
                                <input class="form-check-input" type="checkbox" wire:model="selectedFields" value="tax">
                                {{ __('label.tax') }}
                            </th>
                            <th>
                                <input class="form-check-input" type="checkbox" wire:model="selectedFields" value="food_deduction">
                                {{ __('label.food_deduction') }}
                            </th>
                            <th>
                                <input class="form-check-input" type="checkbox" wire:model="selectedFields" value="advance_deduction">
                                {{ __('label.advance_deduction') }}
                            </th>
                            <th>
                                <input class="form-check-input" type="checkbox" wire:model="selectedFields" value="net_salary">
                                {{ __('label.net_salary') }}
                            </th>
                            <th>
                    ...
                    <tbody class="table-border-bottom-0">
                        ...
                        <tr wire:key="employee-{{ $employee->id }}">
                            ...
                            <td>
                                @if(!$employee?->payroll)
                                    <span class="badge bg-label-warning me-1">Not Calculated</span>
                                @else
                                    @if($employee?->payroll?->status==='pending')
                                        <span class="badge bg-label-warning me-1" style="font-size:10px;">{{ ucfirst($employee?->payroll?->status) }}</span>
                                    @elseif($employee?->payroll?->status==='paid')
                                        <span class="badge bg-label-success me-1" style="font-size:10px;">{{ ucfirst($employee?->payroll?->status) }}</span>
                                    @endif
                                @endif
                            </td>
                            ...
                            <td>{{ $employee?->payroll?->food_deduction }}</td>
                            <td>{{ $employee?->payroll?->advance_deduction }}</td>
                            <td>{{ $employee?->payroll?->net_salary }}</td>
                            <td>{{ $employee?->payroll?->payment_date?->format('Y/m/d') }}</td>
                             @if(!auth()->user()->branch_id)
                ...
                @if(add(Auth::user()->role_ids,$active_menu_id) && count($selected_employees) > 0)
                    
                    <div class="d-flex justify-content-end mt-4 mb-3 px-3 gap-2">
                       @php
                            $has_any_payroll = $this->selected_employees->contains(fn($e) => !is_null($e->payroll));
                            
                            $has_unpaid_payroll = $this->selected_employees->contains(fn($e) => ($e->payroll?->status ?? 'pending') !== 'paid');
                        @endphp
                        ...
                        @if($has_unpaid_payroll)

                            <button type="button"
                                    class="btn btn-primary"
                                    wire:click="savePayroll">
                                <i class="bi bi-save me-1"></i> {{ __('label.calculate') }}
                            </button>
