...
<div>
                    ...
                    <tbody class="table-border-bottom-0">
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                wire:click="showBookSalaryRate({{ $contract->id }})">
        ...
        <div class="modal-dialog modal-lg" branch="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">@if($editMode) {{ __('label.editing') }}  @else {{ __('label.adding') }} @endif {{__('label.temporary_contract')}}</h5>
                    ...
                    <div class="modal-body" >
                        ...
                        <div class="row">
                            ...
                            <div class="col mb-3">
                                ...
                                <div wire:ignore>
                                <select  class="form-control select2" id="temporary_form_position_id" wire:model.lazy ="position_id">
                                    <option value="">{{ __('label.select') }}</option>
                        ...
                        <div class="card mt-3 mb-3" wire:ignore.self>
                    
                            <div class="card-body" wire:key="books-section">

                                <div class="mb-3">
                                    <label>{{ __('label.book') }}</label>

                                    <select class="form-select select2" wire:ignore id="book_id">
                                        <option value="">{{ __('label.select') }}</option>
                                ...
                                @if(!empty($selected_books))
                                    ...
                                    <h6 class="mb-3 text-muted"> {{ __('label.selected_books') }}</h6>

                                    @foreach($selected_books as $index => $book)
                                        <div class="row mb-2 align-items-center border-bottom pb-2" wire:key="selected-book-{{ $book['id'] }}">
                                            ...
                                            <div class="col-md-5">
                                                <input type="number"
                                                    class="form-control"
                                                    placeholder="Amount"
                                                    wire:model.lazy="selected_books.{{ $index }}.amount">
                                            </div>

                                            <div class="col-md-2 text-end">
                                                <button type="button"
                                                        class="btn btn-outline-danger btn-sm"
                                                        wire:click="removeBook({{ $index }})">
                                                    <i class="bx bx-trash"></i>
                                                </button>
                                            </div>

                                            <input type="hidden"
                                                wire:model="selected_books.{{ $index }}.id">
                                        </div>
                                        @error('selected_books.' . $index . '.amount')
    ...
    <div class="modal fade" id="book_salary_rate_modal" tabindex="-1" aria-hidden="true" wire:ignore.self> 
        <div class="modal-dialog modal-lg" branch="document">
            <div class="modal-content">
                <div class="modal-header">
                    
                    <h5 class="modal-title">{{__('label.book_salary_rate')}}</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close" ></button>
                ...
                <div class="modal-body">
                    <table class="table">
                        ...
                        <tbody class="table-border-bottom-0">
                            @foreach($book_salary_rates as $i => $rate)
                            <tr>
                                <td>{{ $i + 1 }}</td>
                                <td>{{ $rate->book?->name }}</td>
                                <td>{{ $rate->amount }}</td>
                            </tr>
                            @endforeach
                        </tbody>
