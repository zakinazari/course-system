...
class MenuList extends Component
{
    ...
    public $active_menu;
    public $modalId = 'menus-addEditModal';
    ...
    public $name_fa,$name_en,$name_pa,$url,$icon,$category,$status = 1,$order,$parent,$grand_parent,$menu_type,$menu_section,$menu_id;
    public $parents = [];
    ...
    public function mount($active_menu_id = null)
    {
        ...
        $this->menu_sections = MenuSection::all();
    ...
    {
        $menus = Menu::with(['type','section','parent','grandParent'])
        ...
        return view('livewire.settings.menus.menu-list',compact('menus'));
    }

    protected function rules()
    {
        $rules = [
            'name_en' => 'required|string|max:255|unique:menus,name_en,' . $this->menu_id,
            'menu_type' => 'required',
            'category' => 'required',
            'status' => 'required',
            'order' => 'required',
    ...
    protected function messages()
    {
        return [
            ...
            'menu_section.required'   => __('label.section.required'),
    ...
    public function edit($id)
    {
        ...
        $this->menu_section = $menu->section_id;
    ...
    public function update()
    {
        if(edit(Auth::user()->role_ids,$this->active_menu_id)){
            ...
            try {
                ...
                Menu::findOrFail($this->menu_id)->update([
                        'name_en' => $this->name_en,
                        'name_fa' => $this->name_fa,
                        'name_pa' => $this->name_pa,
                        'type_id' => $this->menu_type,
    ...
    {   
        ...
        if($menu->type_id==1){
            $sub_menus= Menu::where('parent_id',$menu->id)->get();
            foreach ($sub_menus as $sub) {
                $sub_menu=Menu::find($sub->id);
                $sub_menu->parent_id=$menu->id;
                $sub_menu->grand_parent_id=null;
                $sub_menu->type_id=2;
                $sub_menu->save();
                // sub_menu_sub
                Menu::where('parent_id',$sub->id)->update([
    ...
    public function loadParents($menu_type_id)
    {
        ...
        $this->parents = Menu::where('type_id', $menu_type)
            ...
            ->select('id', 'name_en','name_fa','name_pa')->orderBy('order','ASC')
            ->get();
    }
