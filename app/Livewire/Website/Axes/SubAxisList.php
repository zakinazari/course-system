...
use Auth;
use Carbon\Carbon;
class SubAxisList extends Component
{
    // -------start generals--------------------
    use WithPagination;
    public $perPage = 10;
    ...
    public $active_menu;
    public $modalId = 'sub-axis-list-addEditModal';
    public $table_name='sub_axes';
    public $main_axes = [];
    ...
    public $sub_axis_id,$title_fa,$title_en,$main_axis_id;
    ...
    public function render()
    {
        ...
        return view('livewire.website.axes.sub-axis-list',compact('sub_axes'));
    }
    ...
    {
        $this->resetValidation();
        $main_axis = SubAxis::findOrFail($id);

        $this->sub_axis_id = $id;
        $this->title_fa = $main_axis->title_fa;
        $this->title_en = $main_axis->title_en;
        $this->main_axis_id = $main_axis->main_axis_id;
        $this->editMode = true;
    ...
    public function update()
    {
        if(edit(Auth::user()->role_ids,$this->active_menu_id)){
            $this->validate();
            try {
                $main_axis = SubAxis::findOrFail($this->sub_axis_id);
                $main_axis->update([
                    'title_fa' => $this->title_fa,
                    'title_en' => $this->title_en,
                    'main_axis_id' => $this->main_axis_id,
        ...
        if(delete(Auth::user()->role_ids,$this->active_menu_id)){
            try {
                ...
                SystemLog::create([
                    ...
                    'section' => 'این  محور فرعی  توسط این کاربر حذف شده است. (ID: '.$id.')'.'عنوان: '.$main_axis->title_fa,
                    'type_id' => 4,
                ]);
                $main_axis->delete();
